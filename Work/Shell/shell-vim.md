...
```
1s/11/22:将第一行的 11 换成 2250,100s/old/new/:将第五十到一百行之间的每一个 old 换成 new%s/old/new/g:全文替换g/pattern/s/old/new/g:将全文包含 pattern 的行中的 old 替换为 new# 示例:更改文件中的路径:%s:/home/karl:/home/bear:g # 示例:将第一到第十行的中句号改为分号:1,10s/\./;/g# 示例:将 help 改为 HELP:%s/[Hh]elp/HELP/g# 示例:将一个或多个空格改为一个空格:%s/  */ /g# 示例:将冒号后一个或多个空格改为 2 个空格:%s/:  */:  /g# 删除每一行开头的空白:%/^  *\(.*\)/\1/:%/^  *//# 在每一行开头添加>  :%s/^/>  /# 为后续 6 行添加冒号:.,+5s/$/:/# 把文件变为大写:%s/.*/\U&/
```
...
```
1,10w newfile:将第一至第十行写入newfile340,$ >>newfile:将第三百四十行到文件末尾追加到 newfile
```
...
```
:!date 显示时间:!sh 创建一个 shell:!ls 显示当前文件
```
